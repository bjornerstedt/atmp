...
```{r}
create_payment_plans(state_table, payment_table, globals)
...
analyse_treatment(state_table, payment_table, globals) 

```

```{r}
analyse_treatments(exampleA, over_time = FALSE)
```
```{r}
contract_analysis(exampleA, show_details = TRUE) 
```
