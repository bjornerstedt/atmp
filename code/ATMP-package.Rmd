...
```{r}
example_simple$state_table %>% 
  select( -p_death) %>% 
  filter(treatment == "ATMP") %>% 
  with_titles( example_simple$state_description) %>% 
  flextable_output() %>% 
...
```{r }
example_simple$state_description %>% select(-name, -min, -max) %>% flextable_output()
```
...
```{r, eval=FALSE}
print_transitions(example_simple, "ATMP")
```
...
```{r, message=FALSE}
example_simple$payment_table %>% 
    with_titles( example_simple$payment_description) %>% 
  flextable_output() %>% 
...
```{r}
plot_payment_plans(example_simple)
...
```{r, message=FALSE}
example_simple$payment_description %>% select(-name, -min, -max) %>% flextable_output() 
```
