  ...
  server = function(input, output, session) {
    ...
    output$tro =  
      ...
      DT::renderDataTable( {
        req(vals$filename )
        vals$state_table
        }, selection = 'none', # colnames=get_titles(vals$state_table, indata$treatment_description), 
    ...
    output$cono = 
      ...
      DT::renderDataTable({
        req(vals$filename )
        vals$payment_table
        } , selection = 'none', # colnames=get_titles(vals$payment_table, indata$contract_description), 
                    editable =list(target = 'cell', disable = list(columns = c(0))), options = dtoptions, rownames = FALSE)
